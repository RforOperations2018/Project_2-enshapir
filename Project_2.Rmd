...
AccidInput <- reactive({
    ...
    accidents.server1 <- accidents.load #ckanGeoSQL(url = url)
    accidents.server1$date <- as.Date(accidents.server1$date)
    accidents.server1$number_of_motorist_injured <- as.numeric(as.character(accidents.server1$number_of_motorist_injured))
    accidents.server1$number_of_pedestrians_injured <- as.numeric(as.character(accidents.server1$number_of_pedestrians_injured))
...
output$numInjurM <- renderUI({
        sliderInput(inputId = "numInjurM1", 
                    ...
                    min = min(AccidInput()$number_of_motorist_injured, na.rm = TRUE), 
                    max = max(AccidInput()$number_of_motorist_injured, na.rm = TRUE),
...
output$numInjurP <- renderUI({
        sliderInput(inputId = "numInjurP1", 
                    ...
                    min = min(AccidInput()$number_of_pedestrians_injured, na.rm = TRUE), 
                    max = max(AccidInput()$number_of_pedestrians_injured, na.rm = TRUE),
...
leaflet()%>% 
  ...
  addMarkers(data = accidents.load, lng = ~as.numeric(as.character(longitude)), lat = ~as.numeric(as.character(latitude)), clusterOptions = markerClusterOptions(), popup = ~contributing_factor_vehicle_1, label = ~contributing_factor_vehicle_1,  group = "Accidents") %>% 
addHeatmap(data = accidents.load, lng = ~as.numeric(as.character(longitude)), lat = ~as.numeric(as.character(latitude)), radius = 8, group = "Heat Map") %>% 
